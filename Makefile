...
PROJECT	  := oesip
VERSION   := 0.6.2
DESCR     := "oesip is a modular SIP User-Agent derived from baresip"
...
liboesip.pc:
	@echo 'prefix='$(PREFIX) > liboesip.pc
	@echo 'exec_prefix=$${prefix}' >> liboesip.pc
	@echo 'libdir=$${prefix}/lib' >> liboesip.pc
	@echo 'includedir=$${prefix}/include' >> liboesip.pc
	@echo '' >> liboesip.pc
	@echo 'Name: liboesip' >> liboesip.pc
	@echo 'Description: $(DESCR)' >> liboesip.pc
	@echo 'Version: '$(VERSION) >> liboesip.pc
	@echo 'URL: http://www.creytiv.com/oesip.html' >> liboesip.pc
	@echo 'Libs: -L$${libdir} -loesip' >> liboesip.pc
	@echo 'Cflags: -I$${includedir}' >> liboesip.pc
...
install-shared: $(SHARED) liboesip.pc
	...
	$(INSTALL) -m 0644 liboesip.pc $(DESTDIR)$(LIBDIR)/pkgconfig
...
uninstall:
	...
	@rm -f $(DESTDIR)$(PREFIX)/lib/pkgconfig/liboesip.pc

.PHONY: clean
clean:
	@rm -rf $(BIN) $(MOD_BINS) $(SHARED) $(BUILD) $(TEST_BIN) \
		$(STATICLIB) liboesip.pc
	@rm -f *stamp \
	`find . -name "*.[od]"` \
	`find . -name "*~"` \
	`find . -name "\.\#*"`
...
version:
	@perl -pi -e 's/oesip_VERSION.*/oesip_VERSION \"$(VERSION)"/' \
		include/baresip.h
